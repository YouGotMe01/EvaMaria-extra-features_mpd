...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if f_caption is None:
            f_caption = f"{title}"
        buttons = [[
            InlineKeyboardButton('𝙳𝙴𝚅𝙴𝙻𝙾𝙿𝙴𝚁 👑', url='https://t.me/A4Earner')
            ],[
            InlineKeyboardButton('𝙹𝙾𝙸𝙽 𝙼𝙰𝙸𝙽 𝙲𝙷𝙰𝙽𝙽𝙴𝙻', url='https://t.me/MoviesWorld_Chan_nel')
        ]]
        await query.answer()
        await client.send_cached_media(
            chat_id=query.from_user.id,
        ...
        buttons = [[
            InlineKeyboardButton('⭕️ Group ⭕️', url='https://t.me/+JLuNC2rGfgQ0OGRl'),
...
async def advantage_spell_chok(msg):
    ...
    if not movielist:
        k = await msg.reply("Bro, Check Movie Name In #Google and Try ! Still No Results, Add Movie Year and Try., Again No Results ? It's Not Available In Our Database Or Movie Not Released ! ബ്രോ, മൂവിയുടെ പേര് മാത്രം #Google നോക്കിയിട്ട് ഒന്ന് കൂടെ അടിച്ചു നോക്ക്..!! എന്നിട്ടും കിട്ടിയില്ലെങ്കിൽ പേരിന്റെ കൂടെ മൂവി ഇറങ്ങിയ വർഷം കൂടി അടിച്ചു നോക്ക് 😁 എനിയും കിട്ടിയില്ലെങ്കിൽ ആ മൂവി ഞങ്ങളുടെ ഡാറ്റാബേസിൽ ഇല്ല, അല്ലെങ്കിൽ ആ മൂവി ഇറങ്ങിയിട്ടില്ല എന്ന് മനസ്സിലാക്കുക! 🤗")
        await asyncio.sleep(8)
        await k.delete()
        return
    ...
    await msg.reply("**<b>{message.from_user.mention}, Check Movie Name In [#Google](https://www.google.com/) and Try Again ! Still No Results, It's Not Available In Our Database, We will try to upload soon Or Movie's Masterprint Not Released ! സോദരാ/സോദരീ, മൂവിയുടെ പേര് മാത്രം [#Google](https://www.google.com/) നോക്കിയിട്ട് ഒന്ന് കൂടെ അടിച്ചു നോക്ക്..!! എന്നിട്ടും കിട്ടിയില്ലെങ്കിൽ ആ മൂവി ഞങ്ങളുടെ ഡാറ്റാബേസിൽ ഇല്ല, എത്രയും വേഗം അപ്‌ലോഡ് ചെയ്യാൻ ശ്രമിക്കാം അല്ലെങ്കിൽ ആ മൂവിയുടെ മാസ്റ്റർപ്രിന്റ് ഇറങ്ങിയിട്ടില്ല എന്ന് മനസ്സിലാക്കുക! 🤗</b>**",
                    reply_markup=InlineKeyboardMarkup(btn))
